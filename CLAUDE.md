...
| 21 | **Production warns (does NOT block) on CORS_ORIGIN='*'** | Original v1: throw on wide-open CORS in production. v2 (post-deploy reality check): the strict throw blocked first-time Railway deploys because users don't have the public URL until after the service is created. Now: emit a loud stderr warning if CORS_ORIGIN='*' in production. Operator should still set explicit origin for security, but the server will start and serve traffic. | 2026-05-07 |
